...
function withLinks(text: string, links?: InlineLink[]): ReactNode {
  ...
  return text.split(pattern).map((piece, i) => {
    ...
    const className = "u-underline font-semibold break-words text-gold-700";
...
function contactLine(line: string) {
  if (line.endsWith(site.domainLabel)) {
    ...
    return (
      <>
        {lead}
        <a href={site.url} className="u-underline text-gold-700">
          {site.domainLabel}
        </a>
      </>
    );
    ...
    return (
      <>
        ...
        <a
          ...
          className="u-underline break-all text-gold-700"
        >
          {site.email}
        </a>
      </>
  ...
  if (line.endsWith(site.phone)) {
    ...
    return (
      <>
        {lead}
        <a href={site.phoneHref} className="u-underline text-gold-700">
          {site.phone}
        </a>
      </>
    );
...
function Block({ block }: { block: LegalBlock }) {
      ...
      <ul className="mt-5 space-y-3">
        {block.items.map((item) => (
          ...
          >
            <span
              aria-hidden
              className="u-dot mt-[0.62em] text-gold-600"
            />
  ...
  if (block.kind === "contact") {
    return (
      <address className="mt-6 border-l-2 border-gold-500/70 py-1 pl-5 not-italic sm:pl-6">
    ...
    <main id="main">
      ...
      <section className="u-grain relative isolate overflow-hidden bg-evergreen-800 pb-14 pt-32 lg:pb-18 lg:pt-40">
        <div className="mx-auto w-full max-w-[53.5rem] px-5 sm:px-8">
          ...
          >
            <span aria-hidden className="block h-px w-10 shrink-0 bg-gold-500/80" />
            <span className="u-eyebrow text-gold-400">{site.legalName}</span>
          </p>
        ...
        <div
          aria-hidden
          className="absolute inset-x-0 bottom-0 h-px bg-gold-500/55"
        />
      </section>
      ...
      <div className="bg-ivory-100 pb-20 pt-12 lg:pb-28 lg:pt-16">
        <div className="mx-auto w-full max-w-[53.5rem] px-5 sm:px-8">
          ...
          >
            <h2 className="u-eyebrow text-gold-700">Contents</h2>
            ...
            <ol className="mt-5 sm:columns-2 sm:gap-x-10">
              {doc.sections.map((s) => (
                <li key={s.number} className="break-inside-avoid pb-2.5">
                  ...
                  >
                    <span className="w-6 shrink-0 tabular-nums text-gold-700">
                      {s.number}.
                    </span>
          ...
          {doc.sections.map((s) => (
            ...
            >
              ...
              >
                <span className="text-gold-700">{s.number}.</span>{" "}
                {s.heading}
              </h2>
              <span
                aria-hidden
                className="mt-4 block h-px w-14 bg-gold-600/55"
              />
          ...
          <Reveal className="mt-16 border-t border-rule pt-8 lg:mt-20">
            <h2 className="u-eyebrow text-gold-700">Also in this section</h2>
